...
public class DiaryDao {
	...
	public List<Diary> diaryList(Connection con,PageBean pageBean,Diary s_diary)throws Exception{
		...
		if(s_diary.getUserId() != 0){
			sb.append(" and t1.userId="+s_diary.getUserId());
		}
	...
	public int diaryAdd(Connection con,Diary diary)throws Exception{
		...
		Date releaseDate = DateUtil.formatString(diary.getReleaseDateStr(), "yyyy-MM-dd HH:mm:ss");
	...
	public int diaryUpdate(Connection con,Diary diary)throws Exception{
		Date releaseDate = DateUtil.formatString(diary.getReleaseDateStr(), "yyyy-MM-dd HH:mm:ss");
		java.sql.Date date = new java.sql.Date(releaseDate.getTime());
		String sql="update t_diary set title=?,content=?,typeId=?,releaseDate=? where diaryId=?";
		PreparedStatement pstmt=con.prepareStatement(sql);
		pstmt.setString(1, diary.getTitle());
		pstmt.setString(2, diary.getContent());
		pstmt.setInt(3, diary.getTypeId());
		pstmt.setDate(4, date);
		pstmt.setInt(5, diary.getDiaryId());
		return pstmt.executeUpdate();
	}
